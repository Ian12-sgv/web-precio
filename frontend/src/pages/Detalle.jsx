        ...
        <Card className="border-border bg-card shadow-2xl overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Card className="bg-gradient-to-br from-accent/20 to-accent/5 border-accent/30 p-6 hover:shadow-lg hover:shadow-accent/10 transition-all">
                <div className="flex items-start justify-between mb-2">
                  <DollarSign className="h-5 w-5 text-accent" />
                  <Copy
                    className="h-4 w-4 text-muted-foreground cursor-pointer hover:text-accent transition-colors"
                    onClick={() => copyToClipboard(precioDetalVEF, 'Precio VES')}
                  />
                </div>
                <p className="text-sm text-muted-foreground mb-1">Precio Detal</p>
                <p className="text-3xl font-bold text-foreground tracking-tight">
                  {precioDetalVEF}
                </p>
              </Card>
              ...
              {hasPrecioMayor && (
                <Card className="bg-secondary/50 border-border p-6 hover:shadow-md transition-all">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <Copy
                      ...
                      onClick={() =>
                        copyToClipboard(
                          fmtCurrency(item.PrecioMayor, 'VES'),
                          'Precio Mayor'
                        )
                      }
                    />
                ...
                <Card className="bg-secondary/50 border-border p-6 hover:shadow-md transition-all">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <Copy
                      ...
                      onClick={() =>
                        copyToClipboard(
                          fmtCurrency(item.CostoPromedio, 'USD'),
                          'Costo Promedio'
                        )
                      }
                    />
              ...
              {hasExistencia && (
                <Card className="bg-emerald-50 border-emerald-200 p-6 hover:shadow-lg transition-all sm:col-span-2">
                  <div className="flex items-start justify-between mb-2">
                    <Package className="h-5 w-5 text-emerald-600" />
                    <Copy
                      className="h-4 w-4 text-muted-foreground cursor-pointer hover:text-emerald-600 transition-colors"
                      onClick={() => copyToClipboard(String(item.Existencia), 'Existencia')}
                    />
                  </div>
                  <p className="text-sm text-emerald-700 mb-1">Existencia</p>
                  <p className="text-3xl font-bold text-emerald-700">
                    {item.Existencia} unidades
                  </p>
                </Card>
              )}
            </div>
            ...
            {/* Additional Info */}
            {(item.Categoria || item.Marca || item.Tienda || item.Region) && (
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {item.Categoria && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-foreground font-medium">{item.Categoria}</span>
                    </div>
                  )}
                  {item.Marca && (
                    <div className="flex items-center gap-2">
                      <span className="text-muted-foreground text-sm">Marca:</span>
                      <span className="text-foreground font-medium">{item.Marca}</span>
                    </div>
                  )}
                  {item.Tienda && (
                    <div className="flex items-center gap-2">
                      <span className="text-muted-foreground text-sm">Tienda:</span>
                      <span className="text-foreground font-medium">{item.Tienda}</span>
                    </div>
                  )}
                  {item.Region && (
                    <div className="flex items-center gap-2">
